...
class HarnessEvaluator:
    def __init__(
        self,
        model: Union[
            HarnessModels, lm_eval.api.model.LM, lm_eval.api.model.CachingLM
        ],
        ...
        if isinstance(model, HarnessModels):
            self.lm = model.lm
        elif isinstance(model, lm_eval.api.model.LM) or isinstance(
    ...
    def evaluate_from_model_response(
        ...
        task_dict = HarnessTaskManager.get_task_dict(
            tasks=tasks,
        ...
        if self.lm.rank == 0:
            ...
            results["config"] = {
                ...
                "batch_sizes": (
                    list(self.lm.batch_sizes.values())
